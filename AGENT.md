...
you have access to recall-lite MCP tools. this is a local semantic search engine over the user's indexed files. it's not grep. it doesn't need exact keywords. you describe what you're looking for in natural language and it finds files by meaning.
...
| tool | one-liner |
|------|-----------|
| `recall_search` | semantic + keyword hybrid search, returns ranked results |
| `recall_read_file` | read file content by path, supports line ranges |
| `recall_list_files` | list indexed files with optional extension/path filter |
| `recall_index_status` | check if index exists and how many files/chunks it has |
| `recall_diff` | files changed in a time window (`30m`, `2h`, `1d`, `7d`) |
| `recall_related` | given a file, find semantically similar files |
| `recall_list_containers` | list all search containers and their paths |

## startup sequence

every conversation, before you do anything else:

1. `recall_diff(since: "2h")` — what changed recently? this is your situational awareness
2. `recall_index_status()` — is the index populated? how many files?
...
### recall_search is your primary weapon
...
### when to use recall_search vs grep

| situation | use |
|-----------|-----|
| you know the exact string | grep |
| you know the concept but not the code | recall_search |
| you want all occurrences of a symbol | grep |
| you want to understand how something works | recall_search |
| you're looking for a specific error message | grep |
| you're looking for "where does X happen" | recall_search |
...
```
recall_related(path: "C:\\Users\\...\\src\\indexer\\git.rs", top_k: 5)
```
...
## recall_diff: time-based context

```
recall_diff(since: "1d", show_diff: true)
```
...
## recall_read_file: surgical reads

after search gives you a path+snippet, read the full context:

```
recall_read_file(path: "...", start_line: 50, end_line: 120)
```
...
```
1. recall_index_status()          → how big is the project?
2. recall_list_files()            → what's the file structure?
3. recall_search("main entry point, application startup")
4. recall_search("configuration loading and defaults")
5. recall_related(path: main_file) → what's connected to the entry point?
```

### pattern 2: "find and fix a bug"

```
1. recall_diff(since: "1d")       → what changed recently?
2. recall_search("the behavior the user described")
3. recall_read_file(relevant hit)  → read the actual code
4. recall_related(buggy file)     → find related files that might be affected
```

### pattern 3: "add a new feature"

```
1. recall_search("similar existing feature")  → find the pattern to follow
2. recall_related(similar feature file)        → find all files in that feature
3. recall_list_files(path_prefix: "src/")      → understand project structure
...
```
1. recall_diff(since: "7d", show_diff: true)      → all recent changes
2. for each changed file: recall_related()         → blast radius
3. recall_search("error handling in [changed area]") → check edge cases
```

## what NOT to do

- **don't search an empty index.** check `recall_index_status` first
- **don't use recall_search for exact string matching.** use grep for that
- **don't ignore the `container` parameter.** if the user has multiple containers, you might be searching the wrong one. check with `recall_list_containers`
...
- `recall_related` can be slower on large indexes because it reads embeddings for comparison
- `recall_diff` is fast, it's just checking mtimes

## the philosophy

grep finds strings. recall-lite finds meaning. if you catch yourself constructing 5 different grep queries trying to guess the right keyword, just describe what you're looking for in one recall_search call. that's the whole point.
